function getFromBackend(url, cidade, bairro) {
...
function getPet(page) {
    ...
    console.log(cidade, bairro, especie, genero, instituicao, idade)
    
    try{
        if (cidade != '' || bairro != ''){
            page = 1;
            //data = getFromBackend("http://minhaudocao.com.br:8080/api/evento/search", cidade, bairro, nome);
            //document.querySelector("form").reset();
        } else {
            data = getFromBackend("http://minhaudocao.com.br:8080/api/pet/all", '', '');
        }
        ...
        pets.forEach(element => {
                    if ((RegistroAtual >= (page-1)*limitePagina)&&(RegistroAtual < page*limitePagina)) {
                        ...
                        '<img alt="" class="u-image u-image-default u-image-1" data-image-width="1280" data-image-height="853" src="'+element.uriFotos[0]+'">' +
